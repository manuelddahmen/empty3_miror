 ...
 * <p>
 * Fields:
 * - GENERATE_NOTHING: constant for generating nothing
 * - GENERATE_IMAGE: constant for generating images
 * - GENERATE_MODEL: constant for generating models
 * - GENERATE_OPENGL: constant for generating OpenGL
 * - GENERATE_MOVIE: constant for generating movies
 * - GENERATE_OBJ: constant for generating OBJ files
 * - GENERATE_NO_IMAGE_FILE_WRITING: constant for generating without image file writing
 * - initParams: list of initial parameters
 * - ON_TEXTURE_ENDS_STOP: constant for stopping on texture ends
 * - ON_TEXTURE_ENDS_LOOP_TEXTURE: constant for looping texture on texture ends
 * - ON_MAX_FRAMES_STOP: constant for stopping on max frames
 * - ON_MAX_FRAMES_CONTINUE: constant for continuing on max frames
 * - ENCODER_MONTE: constant for encoder Monte
 * - ENCODER_HUMBLE: constant for encoder Humble
 * - PAL: constant for PAL
 * - HD720: constant for HD720
 * - HD1080: constant for HD1080
 * - UHD: constant for UHD
 * - VGA: constant for VGA
 * - VGAZIZI: constant for VGAZIZI
 * - scene: scene description c
 * - frame: frame description
 * - dynParams: dynamic parameters
 * - couleurFond: background color
 * - z: Z-buffer implementation
 * - encoder: encoder properties
 * - properties: properties of the test object
 * - avif: AVI format
 * - aviOpen: flag for AVI open
 * - filmName: name of the film
 * - idxFilm: film index
 * - unterminable: flag for unterminable
 * - timeStart: start time
 * - lastInfoEllapsedMillis: milliseconds between last info timing
 * - generate: generation version
 * - version: version of the test object
 * - template: template of properties
 * - type: type of the test object
 * - filenameZIP: ZIP filename
 * - fileextZIP: ZIP file extension
 * - resx: X resolution
 * - resy: Y resolution
 * - dir: directory of the test object
 * - ri: rendered image
 * - filename: filename of the test object
 * - fileExtension: file extension of the test object
 * - publish: flag for publishing
 * - isometrique: flag for isometrique
 * - loop: flag for looping
 * - maxFrames: maximum number of frames
 * - text: text of the test object
 * - fileScene: scene file
 * - saveTxt: flag for saving text
 * - binaryExtension: binary file extension
 * - serie: ser
 * - serid: ser ID
 * - initialise: flag for initialization
 * - structure: flag for structure
 * - noZoom: flag for no zoom
 * - sousdossier: subfolder
 * - D3: flag for D3
 * - biic: flag for biic
 * - riG: G-rendered image
 * - riD: D-rendered image
 * - fileG: G-file
 * - fileD: D-file
 * - pause: flag for pause
 * - pauseActive: flag for active pause
 * - directory: directory
 * - zip: flag for ZIP
 * - stop: flag for stop
 * - o: output registration
 * - onTextureEnds: flag for on texture ends
 * - onMaxFrameEvent: event on max frame
 * - dataWriter: data writer for export animation
 * - audioTrack: audio track
 * - isAudioDone: flag for audio done
 * - audioIn: audio input
 * - audioTrackNo: audio track number
 * - videoTrackNo: video track number
 * - fps: frame per second
 * - isVBR: flag for Variable Bit Rate
 * - audioFormat: audio format
 * - dimension: dimension of the test object
 * - name: name of the test object
 * - out: output file
 * - file0: file 0
 * <p>
 * Methods:
 * - TestObjet(): constructor for TestObjet class
 * - TestObjet(ArrayList<TestInstance.Parameter> params): constructor for TestObjet class with parameters
 * - TestObjet(boolean binit): constructor for TestObjet class with initialization flag
 * - z(): get the Z-buffer implementation
 * - setProperties(Properties p): set the properties
 * - getDataWriter(): get the data writer for export animation
 * - getIdxFilm(): get the film index
 * - getSubfolder(): get the subfolder
 * - setResolution(int x, int y): set the resolution
 * - img(): get the image
 * - startNewMovie(): start a new movie
 * - unterminable(): check if the test object is unterminable
 * - isAviOpen(): check if AVI is open
 * - setAviOpen(boolean aviOpen): set the AVI open flag
 * - getGenerate(int GENERATE): get the generate flag for the specified type
 * - runtimeInfoSucc(): get runtime info success message
 * - getO(): get the output registration
 * - afterRenderFrame(): abstract method to be implemented by subclasses
 * - applyTemplate(String template, Properties properties): apply the specified template with the given properties
 * - camera(): get the camera
 * - camera(Camera c): set the camera
 * - D3(): check if the test object is D3
 * - description(String d): set the description
 * - directory(): get the directory
 * - ecrireImage(RenderedImage ri, String type, File fichier): write the image to the specified file
 * - exportFrame(String format, String filename): export the frame to the specified format and filename
 * - finit(): abstract method to be implemented by subclasses
 * - frame(): get the frame
 * - getDynParams(): get the dynamic parameters
 * - getFile(): get the file
 * - getFilename(): get the filename
 * - setFilename(String fn): set the filename
 * - getGenerate(): get the generate flag
 * - setGenerate(int generate): set the generate flag
 * - getInitParams(): get the initial parameters
 * - getMaxFrames(): get the maximum number of frames
 * - setMaxFrames(int maxFrames): set the maximum number of frames
 * - getResx(): get the X resolution
 * - setResx(int resx): set the X resolution
 * - getResy(): get the Y resolution
 * - setResy(int resy): set the Y resolution
 * - ginit(): abstract method to be implemented by subclasses
 * - init(): initialize the test object
 * - dateForFilename(Date date): get the formatted date string for filename
 * - isometrique(boolean isISO): set the isometrique flag
 * - isometrique(boolean isISO, boolean noZoom): set the isometrique flag and the no zoom flag
 * - isPause(): check if the test object is paused
 * - isPauseActive(): check if the test object is active paused
 * - isSaveBMood(): check if the test object is in save b mood
 * - isStructure(): check if the test object has structure
 * - setStructure(boolean structure): set the structure flag
 * - loop(): check if the test object is looping
 * - loop(boolean isLooping): set the looping flag
 * - nextFrame(): proceed to the next frame
 * - nextFrame2UnknownDiplicate(): proceed to the next frame with an unknown duplicate
 * - PAUSE(): pause the test object
 * - publishResult(): publish the result
 * - getPublish(): get the publish flag
 * - setPublish(boolean publish): set the publish flag
 * - publishResult(boolean publish): publish the result with the specified flag
 * - reportException(Exception ex): report an exception
 * - reportPause(boolean phase): report a pause
 * - reportStop(): report a stop
 * - reportSuccess(File film): report a success with the specified film file
 * - copyResources(): copy the resources
 * - addAudioFile(File audio): add an audio file
 * - initCompiler(): initialize the compiler
 * - run(): run the test object
 * - saveBMood(boolean b): save the b mood
 * - scene(): get the scene
 * - paintingAct(Representable representable, PaintingAct pa): paint the specified representable with the specified painting act
 * - closeView(): close the view
 * - scene(Scene load): set the scene
 * - serid(): get the ser ID
 * - set3D(boolean b3D): set the 3D flag
 * - setCouleurFond(ITexture tColor): set the background color
 * - setDynParameter(TestInstance.Parameter parameter): set the dynamic parameter
 * - setFileExtension(String ext): set the file extension
 * - STOP(): stop the test object
 * - testScene(): abstract method to be implemented by subclasses for testing the scene
 * - testScene(File f): abstract method to be implemented by subclasses for testing scene with the specified file
 * - writeOnPictureAfterZ(BufferedImage bi): write on the picture after Z
 * - writeOnPictureBeforeZ(BufferedImage bi): write on the picture before Z
 * - getFolder(): get the folder
 * - unterminable(boolean b): set the unterminable flag
 * - getZ(): get the Z-buffer
 * - onTextureEnds(ITexture texture, int texture_event): event on texture ends
 * - onMaxFrame(int maxFramesEvent): event on max frame
 * - getInstance(): get the TestObjet instance
 * - getDimension(): get the dimension
 * - setDimension(Resolution dimension): set the dimension
 * - setName(String name): set the name
 * - v2main(): get the main color
 * - getWrittenFile(): get the written file
 * - getFps(): get the frame per second
 * - setFps(int fps): set the frame per second
 * - setZ(ZBufferImpl z): set the Z-buffer implementation
 * - getDir(): get the directory
 * <p>
 * Superclasses:
 * - java.lang.Object
 * - one.empty3.library.core.testing.Test
 * - java.lang.Runnable
 */
